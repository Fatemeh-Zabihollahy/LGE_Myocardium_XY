...
for n in range(18): 
    ...
    for slice_no in range (z):
        ...
        for a in range (x):
            for b in range (y):
                if lge_slice[a,b] > 1000:  # remove the outlier pixels
                    lge_slice[a,b] = numpy.median(lge_slice)
        lge_slice = (lge_slice-lge_slice.min())/(lge_slice.max()-lge_slice.min())  # normalize each slice
...
#%%
    ...
    # perform padding to resize all images in XY dirction    
    x_pad = int(x_unet - x)
    y_pad = int(y_unet - y)
    ...
    for page in range(0,z):    
        ...
        if (numpy.max(myo_slice) != 0):
            ...
            # Artficially augmenting images and masks          
